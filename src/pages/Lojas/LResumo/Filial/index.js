    ...
    useEffect(() => {
        async function getFiliais() {
            ...
            await api.get(`filiais/${dtFormatada(dataFiltro)}`)
                .then(filiais => {
                    const fil = filiais.data.filter((dep) => (dep.Departamento === 1));
                    setFiliais(fil);
                    setLoading(false);
                })
                .catch(err => {
                ...
                <ScrollView horizontal={true} showsHorizontalScrollIndicator={false}>
                    <DataTable>
                        <DataTable.Header style={{ backgroundColor: '#E5E5EA' }}>
                            ...
                            <DataTable.Title style={styles.colgrande}><Text style={styles.textAlign}>Faturamento</Text></DataTable.Title>
